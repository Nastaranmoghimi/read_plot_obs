...
name = 'maryland'
region[name]['xmin']  = -80.1-1
region[name]['xmax']  = -75+1
region[name]['ymin']  =  37.8-1
region[name]['ymax']  = 40.25+1
...
#purp_infos = pd.read_excel(purple_stations_info_file)
#
purp_outside = find_purple_outside()
...
for key in purp_outside.keys():
    ...
    try:
        lons_out.append (float(purp_outside [key]['lon']) )
        lats_out.append (float(purp_outside [key]['lat']) )
        label_out.append(      purp_outside [key]['label'])
        id_out.append(key)
    except:
        print (key)
        pass

lons_out  = np.array(lons_out)
lats_out  = np.array(lats_out)
label_out = np.array(label_out)
id_out    = np.array(id_out)

purp_ind = get_ind_poly(lons_bound = lons_bou,lats_bound=lats_bou,lons =lons_out ,lats=lats_out)


id_sta_purp   = id_out   [purp_ind]
lon_sta_purp  = lons_out [purp_ind]
lat_sta_purp  = lats_out [purp_ind]
labels_purp   = label_out[purp_ind]
#
data_sta = np.c_[id_sta_purp,lon_sta_purp,lat_sta_purp,labels_purp]
...
fig,ax = make_map()                                             
ax.set_title( name.capitalize() + ' Outdoor Stations')
ax.scatter(x = lon_sta_purp , y =lat_sta_purp ,s=20,lw=0, c= 'purple',alpha=0.85,label = 'Purple air')
